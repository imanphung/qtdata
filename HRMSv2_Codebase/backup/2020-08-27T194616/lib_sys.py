...
#List of libs
lib_global = ['lib_sys', 'lib_tempo', 'lib_memberinfo', 'lib_bot']
        ...
        for filepath in list_filepath:
            try:
                ...
                df_server = pd.read_sql("select * from {} where to_char(FUNCTION) in {} and LIB like '{}'"\
                    .format(utils.db_functions, list_func, libname), utils.ENGINE)
                ...
                utils.ENGINE.execute(
                    ...
                    .format(utils.db_functions, list_func, libname)
                )
                
                df_local.to_sql(utils.db_functions, utils.ENGINE, if_exists = 'append', index = False)
...
def get_function(libname = None):
    ...
    if libname is None:
        df = pd.read_sql("select * from {} where LIB not like 'sys_file'".format(utils.db_functions), utils.ENGINE)
        df.columns = [x.upper() for x in df.columns]

        lib_list = list(df['LIB'].unique())
    else:
        df = pd.read_sql("select * from {} where LIB like '{}'".format(utils.db_functions, libname), utils.ENGINE)
        df.columns = [x.upper() for x in df.columns]
            ...
            try:
                if filename in sys_global:
                    ...
                    df_server = pd.read_sql("select * from {} where FUNCTION like '{}' and LIB like 'sys_file'"\
                        .format(utils.db_functions, filename), utils.ENGINE) 
                    df_server.columns = [x.upper() for x in df_server.columns]
                    ...
                    utils.ENGINE.execute(
                        ...
                        .format(utils.db_functions, filename)
                    )
...
def get_sysfile(sysfile = None):
    ...
    if sysfile is None:
        df = pd.read_sql("select * from {} where LIB like 'sys_file'".format(utils.db_functions), utils.ENGINE)
    else:
        df = pd.read_sql(
            "select * from {} where FUNCTION like '{}' and LIB like 'sys_file'"\
            .format(utils.db_functions, sysfile), utils.ENGINE
        ) 
    df.columns = [x.upper() for x in df.columns]
...
def backup(list_files):
    backup_folder = './backup/{}'.format(datetime.now().strftime('%Y-%m-%dT%H%M%S'))
