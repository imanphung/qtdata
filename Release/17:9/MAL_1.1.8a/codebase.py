    ...
    while True:
        try:
            ...
            if event == '-RESET-':
                ...
                kill_processtree('Capture Images', capimgs_pid)
                kill_processtree('TempoNote', temponote_pid)
                reset_pid = execute_command(
                    'Reset',                                   
                    'python', './popup_tempo.py', '-r on',
                    '-temponotepid {}'.format(temponote_pid),
                    '-capimgspid {}'.format(capimgs_pid),
                    '-s {}'.format(utils.server_status)
                )
                #event = sg.TIMEOUT_KEY
