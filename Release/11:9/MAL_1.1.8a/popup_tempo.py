...
def add_checkname(name):
    level_checked = pd.read_sql("select CLASS from {} where CODE like '{}'"\
                     .format(utils.db_memberinfo, name), utils.ENGINE)['class'][0]
    ...
    if int(my_level) > int(level_checked):
        return "You can't check this member"
    else:
        file_path = utils.path_checkname
        str_checkname = None
        if not os.path.exists(file_path):
...
def create_window_monitor(str_checkname):
    ...
    if len(list_checkname) >= 3:
        layout = [
            [sg.Column(check_user, size = (250, 260), vertical_scroll_only = True, scrollable = True)]
        ]
    else:
        layout = check_user
...
#                        "and DATETIME in (select max(DATETIME) from {} where to_char(NAME) in {} group by to_char(NAME))"\
#                        .format(utils.db_tempomonitor, str_checkname, utils.db_tempomonitor, str_checkname), utils.ENGINE)
#                    df_monitor.columns = [x.upper() for x in df_monitor.columns]
                    df_monitor1 = pd.read_sql(
                        "select * from {} where to_char(NAME) in {} "\
                        "and DATETIME in (select max(DATETIME) from {} group by to_char(NAME))"\
                        .format(utils.db_tempomonitor, str_checkname, utils.db_tempomonitor, str_checkname), utils.ENGINE)
                    df_monitor1.columns = [x.upper() for x in df_monitor.columns]
                    df_monitor1.drop_duplicates(subset=['NAME'],keep='last',inplace=True)
                    #df_monitor.groupby(['NAME'], sort=False)['DATETIME'].max()
                    #df_monitor.sort_values('DATETIME', ascending=False).drop_duplicates('NAME')
                    #Current Task
                    df_task = pd.read_sql(
