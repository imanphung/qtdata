...
#         return 'View in output directory'
def get_image(username = utils.username, date = utils.current_date.strftime('%Y-%m-%d'), mode = 'daily', group = None):
    # level_result = utils.check_my_level(username)
    # if level_result == 0:
    #     return "You are not allowed to check on this member"
    # else:
        date_for_path = datetime.strptime(date, '%Y-%m-%d').strftime('%d-%b-%y').upper()
        path = utils.path_output + '{}_IMAGE_REPORTS/'.format(date_for_path)
        if not os.path.exists(path):
            os.makedirs(path)
        ...
        try:
            if group is not None:
                if group == 'all':
                    df_username = pd.read_sql("select code, name from {} where department not like '{}' ORDER BY to_char(name) ASC"\
                                                .format(utils.db_memberinfo, 'qt-Technology-Intern'), utils.ENGINE)
                else:
                    df_username = pd.read_sql("select code, name from {} where department like '{}' ORDER BY to_char(name) ASC"\
                                                .format(utils.db_memberinfo, group), utils.ENGINE)
                count = len(df_username)
            else:
                count = 1

            for i in range(count):
                if group is not None:
                    username = df_username.loc[i, 'code']
                    filename = df_username.loc[i, 'name']
                    if group == 'all':
                        groupname = df_username.loc[i, 'department']
                    else:
                        groupname = group
                    path_username = path + '{}_{}/'.format(groupname, datetime.now().strftime('%H%M%S'))
                else:
                    path_username = path + '{}_{}/'.format(username, datetime.now().strftime('%H%M%S'))
                if not os.path.exists(path_username):
                    os.makedirs(path_username)
                
                for j in range(num_days):
                    if mode == 'weekly':
                        date = (monday + timedelta(days = j)).strftime('%d-%b-%y').upper()
                    elif mode == 'monthly':
                        if (day1 + timedelta(days = j)).month != month:
                            break
                        date = (day1 + timedelta(days = j)).strftime('%d-%b-%y').upper()
                    ...
                    date_position = (20,10)
                    #path_files = path_username +'{}_{}.gif'.format(filename, date)
                    with imageio.get_writer(path +'{}_{}.gif'.format(filename, date), mode='I', fps=5) as writer_screen:
                        for idx, row in df.iterrows():
                            screen = imageio.imread(base64.b64decode(row['SCREEN']))
                            webcam = imageio.imread(base64.b64decode(row['WEBCAM']))
                            concat_image = cv2.hconcat([webcam,screen])
        ...
        log_temponote.warning('View in output directory')
        return 'View in output directory'
