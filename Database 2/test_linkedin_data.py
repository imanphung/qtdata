...
try:
	print(engine.execute('SELECT count(*) FROM LINKEDIN_DB').fetchall())
	# engine.execute('CREATE TABLE theater\
	# 					    (id VARCHAR2 (255)\
	# 					    json_document BLOB\
	...
	#engine.execute("DROP TABLE j_purchaseorder")
	...
	#			)
	engine.execute("INSERT INTO j_purchaseorder (id, date_loaded, po_document)")
...
#         # conn.commit()
#         df_report.to_sql('LINKEDIN_DB', engine, if_exists='append', index = False)
#         finish = datetime.datetime.now()
#         log += 'Upload successfully\n'
#         log += 'Upload time: ' + str((finish - start).total_seconds()) + '\n'
